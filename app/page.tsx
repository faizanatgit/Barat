...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/Baraat.mp3", { method: "HEAD" }).then((r) => setMusicAvailable(r.ok)).catch(() => setMusicAvailable(false));
    fetch("/baraat.mp4", { method: "HEAD" }).then((r) => setVideoAvailable(r.ok)).catch(() => setVideoAvailable(false));
  }, []);
  useEffect(() => {
  ...
  return <main>
    <section id="home" className="hero">
      {videoAvailable && (<video className="hero-video" autoPlay loop muted playsInline poster="/baraat-botanical-backdrop.png"><source src="/" type="video/mp4" /></video>)}
      <div className="hero-overlay" /><div className="hero-content">
        <p className="hero-event">Baraat</p><p className="hero-host">MR. &amp; MRS. AZHAR BASHIR</p><p className="hero-copy">CORDIALLY INVITE YOU<br />TO CELEBRATE THE BARAAT NIGHT<br />OF THEIR BELOVED SON</p>
        <h1>Muhammad Zeeshan Azhar Malik</h1><p className="program">Programme In-sha-Allah</p><div className="hero-date"><span /><b>Friday</b><span /></div><p>6th &nbsp;|&nbsp; NOVEMBER &nbsp;|&nbsp; 2026</p><p>BARAAT CELEBRATION</p>
      </div><a className="scroll-cue" href="#countdown">⌄<small>Scroll to discover</small></a>
      <audio ref={audio} autoPlay loop preload="auto" src="/Baraat.mp3" onEnded={() => setPlaying(false)} />
